...
class Repeater(nn.Module):
    def __init__(
        ...
        self.x_min_ = None
        self.x_scale_ = None
        self.x_data_min_ = None
        self.x_data_max_ = None
        self.x_data_range_ = None
        self.x_n_features_in_ = None
        self.x_n_samples_seen_ = None
        self.y_min_ = None
        self.y_scale_ = None
        self.y_data_min_ = None
        self.y_data_max_ = None
        self.y_data_range_ = None
        self.y_n_features_in_ = None
        self.y_n_samples_seen_ = None
    ...
    def train_loop(self, dataloader):
        ...
        for batch, (x, y) in enumerate(dataloader):
            ...
            x = torch.FloatTensor(x.float()).to(self.device)
            y = torch.FloatTensor(y.float()).to(self.device)
    ...
    def test(self, x, initial_value=None):
        ...
        x = torch.FloatTensor(np.expand_dims(x, axis=0)).to(self.device)
        y = torch.FloatTensor(np.expand_dims(y, axis=0)).to(self.device)
        ...
        with torch.no_grad():
            for i in range(x.shape[1]):
                ...
                self.y_tick.append(np.squeeze(y[:, k:, :].cpu().numpy(), axis=0)[-1])
...
class Evaluator:
    ...
    def measure(self, y_true, y_pred):
        self.metrics_dict["explained_variance_score"] = (
        ...
        self.metrics_dict["mean_absolute_percentage_error"] = (
